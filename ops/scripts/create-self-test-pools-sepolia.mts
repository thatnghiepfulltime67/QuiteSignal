...
const policies = [
  ['greater-or-equal', '200000000000', 7_200, 2],
  ['less-than', '200000000000', 14_400, 2],
] as const;
...
interface RegistryRecord {
  poolAddress: string;
  policy: { comparison: string; threshold: string; minutes: number; gate: number };
  startedAt: string;
}
...
function loadLedger(path: string): Ledger {
  ...
  if (ledger.schemaVersion !== 1 || ledger.chainId !== 11155111 || !Array.isArray(ledger.entries))
...
async function main(): Promise<void> {
  ...
  if (write && process.env.CONFIRM_SEPOLIA_WRITE !== 'yes')
    fail('CONFIRM_SEPOLIA_WRITE=yes is required for a Sepolia write.');

  const deployment = JSON.parse(
    readFileSync('deployments/sepolia/releases/DEP-02.json', 'utf8'),
  ) as {
  ...
  const [chainId, factoryCode, collateralCode, feedCode, balance, fees] = await Promise.all([
    ...
    publicClient.getCode({ address: factory.address as Address }),
    publicClient.getCode({ address: collateral.address as Address }),
    publicClient.getCode({ address: feed }),
    publicClient.getBalance({ address: account.address }),
    publicClient.estimateFeesPerGas(),
  ]);
  if (chainId !== 11155111) fail('The configured RPC is not Ethereum Sepolia.');
  if (!factoryCode || keccak256(factoryCode) !== factory.runtimeCodeHash)
    fail('The factory runtime does not match the deployment record.');
  if (!collateralCode || collateralCode === '0x' || !feedCode || feedCode === '0x')
    fail('The wrapper or feed has no Sepolia runtime.');
  ...
  if (!write) {
    console.log(
      JSON.stringify({
        ...
        conservativePoolGasLimit: MAX_POOL_GAS.toString(),
      }),
    );
    return;
  }
  ...
  const evidencePath = resolve('evidence/sepolia/WEB-15-POOLS.json');
  const registry = JSON.parse(readFileSync(registryPath, 'utf8')) as RegistryRecord[];
  const priorEvidence = existsSync(evidencePath)
    ? ((JSON.parse(readFileSync(evidencePath, 'utf8')) as { pools?: unknown[] }).pools ?? [])
    : [];
  const created: Array<{
    poolAddress: string;
    adapterAddress: string;
    adapterTransactionHash: string;
    poolTransactionHash: string;
    policy: RegistryRecord['policy'];
    startedAt: string;
  }> = [];
  const send = async (id: string, request: { data: Hex; to?: Address }) => {
    ...
    if (totalSpend(ledger) + maximumCost > BigInt(ledger.maxTotalSpendWei))
      fail(`${id} exceeds the remaining Sepolia budget.`);
  ...
  for (const [index, [comparison, threshold, minutes, gate]] of policies.entries()) {
    ...
    const config = {
      confidentialCollateral: collateral.address as Address,
    ...
    const poolData = encodeFunctionData({
      abi: factoryAbi,
    ...
    if (/^0x0{40}$/i.test(pool))
      fail(`Pool ${index + 1} is absent from the factory after confirmation.`);
    const verified = await createViemProtocolPublicReader(publicClient).readConfig(
      publicAddress(pool),
    );
    if (
      verified.confidentialCollateral.toLowerCase() !== collateral.address.toLowerCase() ||
      verified.resolutionAdapter.toLowerCase() !== adapter.toLowerCase() ||
      verified.deadline !== deadline ||
      verified.kMin !== gate
    )
      fail(`Pool ${index + 1} configuration did not verify.`);
    const record = {
      poolAddress: pool,
      policy: { comparison, threshold, minutes, gate },
      startedAt: startedAt.toString(),
    };
    if (!registry.some((known) => known.poolAddress.toLowerCase() === pool.toLowerCase()))
      registry.push(record);
    writeFileSync(registryPath, `${JSON.stringify(registry, null, 2)}\n`);
    created.push({
      ...record,
      adapterAddress: adapter,
      adapterTransactionHash: adapterReceipt.transactionHash,
      poolTransactionHash: ledger.entries.at(-1)?.transactionHash ?? '',
    ...
    console.log(
      JSON.stringify({
        status: 'verified',
        index: index + 1,
        condition: policyLabel(comparison, threshold),
        poolAddress: pool,
      }),
    );
  }
  const poolRecords = [...priorEvidence, ...created];
  writeFileSync(
    evidencePath,
    `${JSON.stringify({ workItemId: WORK_ITEM_ID, chainId: 11155111, createdAtUtc: new Date().toISOString(), pools: poolRecords }, null, 2)}\n`,
  );
  console.log(
    JSON.stringify({
      status: 'complete',
      poolCount: created.length,
      remainingBudgetWei: (BigInt(ledger.maxTotalSpendWei) - totalSpend(ledger)).toString(),
    }),
  );
}
