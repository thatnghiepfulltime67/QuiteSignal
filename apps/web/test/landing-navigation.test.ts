...
test('T-WEB-02-LANDING-01: landing and navigation orient users before wallet use', () => {
  ...
  assert.match(source, /class="market-pool-filter"/);
  assert.match(source, /Filter: \$\{marketPoolFilterLabel\(marketPoolFilter\)\}/);
  assert.match(source, /id="market-pool-filter"/);
  assert.match(source, /marketPoolFilter = value/);
  ...
  assert.match(styles, /\.market-pool-filter \{/);
  assert.match(styles, /\.market-pool-filter\[open\] summary::after/);
